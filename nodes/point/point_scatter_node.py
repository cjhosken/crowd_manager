...
class CrowdManager_PointScatterNode(bpy.types.Node, CrowdManagerBaseNode):
    ...
    def update(self):
        ...
        if len(self.outputs) > 0 and self.outputs[0].links:
            ...
            self.outputs[0].points = point_list_to_string(points)
...
#################################################
    ...
    def generateGridPoints(self):
        ...
        for x in range(w):
            for y in range(l):
                for z in range(h):
                    pnt = [(x*gs, y*gs, z*gs), (0.0, 0.0, 0.0)]
                    points.append(pnt)
        return points
        ...
        while len(spawn_points) > 0:
            ...
            for i in range(self.num_sample_before_rejection):
                ...
                if self.isValid(candidate, cell_size, points, grid):
                    points.append([(candidate.x, candidate.y, 0), (0, 0, 0)])
            ...
            for x in range(int(search_start.x), int(search_end.x + 1)):
                for y in range(int(search_start.y), int(search_end.y + 1)):
                    ...
                    if point_index != -1:
                        ...
                        point = Vector((points[point_index][0][0], points[point_index][0][1], 0))
                        dist_vec = cand - point
